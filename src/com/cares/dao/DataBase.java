...
public class DataBase {
	...
	public int insertIntoDB(ArrayList e) {
		...
		try {
			...
			sql = "INSERT into registration2 VALUES(sequence05.nextval,?,?,?,?,?,?,?,?,?)";
	...
	public int login(String pa , String adhar ) {
		...
		try {
			...
			sql = "select  PASSWORD ,FNAME  from registration2 where AADHAR = ? ";
			stm = con.prepareStatement(sql);
	...
	{		
		...
		try {
			...
			String sql = "select FNAME ,LNAME,EMAIL,PHONE,GENDER, ADDRESS , DOB ,PASSWORD from registration2 where AADHAR = ? ";
			stm = con.prepareStatement(sql);
			stm.setString(1,adhar);
			rs= stm.executeQuery();
	...
	public int UpdateDetails(ArrayList e , String Adhar)
	{
		...
		try {
			...
			String sql = "UPDATE registration2 SET FNAME = ? , LNAME = ? ,PHONE = ? ,EMAIL = ? , DOB = ? , ADDRESS = ?   where AADHAR = ? ";
			stm = con.prepareStatement(sql);
	...
	public int forgotPass(String Adhar, String pass1)
	{
		...
		try {
			...
			String sql = "UPDATE registration2 SET PASSWORD = ?  where AADHAR = ? ";
			stm = con.prepareStatement(sql);
	...
	public  void bookingCheck(String Adhar ) 
	{
		...
		try {
			...
			sql = "select FIRST_DOSE,FIRST_DOSE_DATE,	SECOND_DOSE,SECON_DOSE_DATE  from CovidVaccinabooking1 where AADHAR = ? ";
			stm = con.prepareStatement(sql);
			stm.setString(1, Adhar );
			rs = stm.executeQuery();
			 while (rs.next()) 
	...
	public   int bookingUpdate(String Adhar,String Vaccine , String date ,String center )
	{
		...
		try {
			...
			sql = "INSERT into CovidVaccinabooking1(VACCINE,FIRST_DOSE,FIRST_DOSE_DATE,CENTER, AADHAR) VALUES(?,?,?,?,?) ";
			    ...
			    if(true) {
			    	sql = "UPDATE CovidVaccinabooking1 set VACCINE = ? ,SECOND_DOSE = ? ,SECON_DOSE_DATE = ? ,CENTER = ? where AADHAR = ? ";
			    }else
			    {
	...
	public int markAsTaken(String Adhar)
	{
		...
		try {
			...
			if(FIRST_DOSE.equals("Booked")&&SECOND_DOSE==null) {
			 ...
			 sql = "UPDATE CovidVaccinabooking1 set FIRST_DOSE = ?  where AADHAR = ? ";
				...
				JOptionPane.showMessageDialog(null,"SECOND_DOSE not booked");
					 ...
					 sql = "UPDATE CovidVaccinabooking1 set SECOND_DOSE = ?  where AADHAR = ? ";	
				}
				else {
	...
	public void addSlots(String Vaccine , String center, String slots) {
		PreparedStatement stm = null;
		Connection con = null;
		Statement stmt = null;
		int a=0;
		...
		try {
			...
			sql = "UPDATE Vaccine_slots  SET SLOTS = ? WHERE VACCINE_NAME = ? AND  CENTER_NAME = ? ";
			stm = con.prepareStatement(sql);
			stm.setInt(1,Integer.parseInt(slots));
			stm.setString(2,Vaccine);
			stm.setString(3,center);
			count =   stm.executeUpdate();
			if(count==1) {
			...
			{
				JOptionPane.showMessageDialog(null, "Slots Dosen't Updated" );
			}
		}catch(Exception g) {}
  }
}